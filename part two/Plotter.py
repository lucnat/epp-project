...
def getStack(var, samples, excludeSig = False):
    ...
    for s in samples:
        if not os.path.exists( s + "_histos.root"): 
            ...
            if(excludeSig and s == "wjets"): continue
...
def plotVar(var, samples,isData = False, logScale = False ):
    ...
    if not os.path.exists( "ttbar_histos.root"): 
        ...
        h = f.Get(var)
        setStyle(h, samp["wjets"], 0, 0)
        h.SetLineColor(samp["wjets"])
        h.SetLineWidth(2)
        h.Draw("histsame")
        leg.AddEntry(h, "wjets", "L")
    ...
    if(isData): 
        ...
        c.SaveAs("generated/"+var + ".pdf")
    else: c.SaveAs("generated/"+var + "_MC.pdf")
...
def plotVarNorm(var, samples, logScale = False):
    ...
    leg.Draw("SAME")
    c.SaveAs("generated/"+var + "_Norm_MC.pdf")
...
def plotShapes(var, samples, logScale = False):
    ...
    if not os.path.exists( "ttbar_histos.root"): 
        ...
        if(h.Integral()!=0.):h.Scale(1./h.Integral())
        h.Draw("histsame")
        leg.AddEntry(h, "Signal (wjets)","L")

    leg.Draw("SAME")
    c.SaveAs("generated/"+var + "_Shape_MC.pdf")
